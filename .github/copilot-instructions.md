...
## Debugging
- Quick dev (open DevTools):
  ```bash
  # development with DevTools off by default
  pnpm dev:ui-tars

  # open DevTools + sourcemaps and remote debugging port
  pnpm --filter ui-tars-desktop debug
  pnpm --filter ui-tars-desktop debug:w
  # OR (inside apps/ui-tars)
  pnpm debug
  ```

 - If Electron fails to start in a headless/container environment, install missing system libs (example for Debian/Ubuntu):
  ```bash
  sudo apt-get update && sudo apt-get install -y libatk1.0-0 libgdk-pixbuf2.0-0 libgtk-3-0 libx11-6 libxss1 libnss3
  ```

 - Common runtime errors:
   - Missing native libs (e.g. `libatk-1.0.so.0`): install system dependencies above.
