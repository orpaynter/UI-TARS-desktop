...
program
  .name(process.env.NAME || 'mcp-server-nut-js')
  .description(process.env.DESCRIPTION || 'MCP server for nut-js')
  ...
  .action(async (options) => {
    try {
      ...
      if (options.port || options.host) {
        ...
        console.debug('Computer Use MCP Server running on stdio');
      }
    } catch (error) {
