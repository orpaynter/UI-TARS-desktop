...
export class ShareService {
  ...
  async shareSession(
    ...
    try {
      ...
      // Process workspace images in events if uploading
      ...
      const shareHtml = this.generateShareHtml(processedEvents, metadata);
  ...
  private async processWorkspaceImages(
    ...
    console.log('processWorkspaceImages');
  ...
  private async uploadWorkspaceImage(absolutePath: string, relativePath: string): Promise<string> {
    ...
    // formData.append('sessionId', 'image');
    formData.append('originalPath', relativePath);

    try {
      // FIXME: storage.provider.
      ...
      throw new Error('Invalid response from share provider for image upload');
    } catch (error) {
