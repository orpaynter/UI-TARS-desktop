...
export const ActionBar: React.FC<ActionBarProps> = ({ sessionId, className = '' }) => {
  ...
  const renderPlanButton = () => {
    ...
    return (
      <motion.button
        ...
        className={`h-10 flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-white/90 to-gray-50/90 dark:from-gray-800/90 dark:to-gray-700/90 rounded-full border border-gray-200/60 dark:border-gray-600/40 shadow-sm hover:shadow-md ${backdropBlurClasses} transition-all duration-200`}
      >
        <div className="flex items-center gap-2">
          ...
          <div
            className={`px-2 py-0.5 rounded-full text-xs font-medium ${
              isComplete ? statusIndicatorClasses.success : statusIndicatorClasses.processing
            }`}
          >
  ...
  return (
    <AnimatePresence>
      <motion.div
        {...fadeInUpVariants}
        className={`mx-0 mb-3 p-3 bg-[#f9fafb] dark:bg-slate-800/60 ${backdropBlurClasses} rounded-xl border border-gray-200/60 dark:border-gray-700/50 ${className}`}
      >
